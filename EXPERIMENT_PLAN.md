...
| 4b | Q (punctuation-surface-form ablation — added `DECISION_REGISTER.md` M2-3, resolving the punctuation-stripping confound discovered in Milestone 2's audit) | `run_shortcut_analysis.py --mode punctuation-ablation`, which dispatches to `research/src/experiments/punctuation_ablation.py` (also directly runnable, and the home of the Q-specific transform — **never** `research/src/data/clean.py`, which is the shared train/serve path) | Step 4. XLM-R only, Ax-to-Grind→Notri-Fact direction only, 3 seeds — retrain on punctuation-stripped Ax-to-Grind, re-evaluate zero-shot on Notri-Fact, report the macro-F1 delta against F/G's same-direction XLM-R result. Stripping = every character that is not a Unicode letter, digit or whitespace, replaced by a space, then whitespace collapsed (M2-3's implementation note). Writes `Q_xlm-roberta-base_notri_fact_holdout_seed{42,123,2026}.json` + predictions siblings; checkpoints branch as `seed-<n>-punct-ablation` in the existing XLM-R staging repo |
| 5 | I (length-ablation, replicate the 50-word cap design) | `run_shortcut_analysis.py --mode length-ablation`, which dispatches to `research/src/experiments/length_ablation.py` (also directly runnable; the cap lives there, **never** in `research/src/data/clean.py`) | Step 3. XLM-R (3 seeds) + best classical (**B/tfidf_svm**, 0.8835 > A's 0.8755 — singular, per Part 11), Ax-to-Grind **in-domain only**. Word-cap **sweep `{25, 50, 100, 200}` with 50 the required primary point** — resolves Part 1/11 ("a fixed cap", singular) against Part 31 figure 10 ("a curve"); see `DECISION_REGISTER.md` M5-5. Cap = keep the first N whitespace-delimited words (right-truncation, matching M2-1/M4-1), applied to **train and eval alike**. Writes `I_xlm-roberta-base_ax_to_grind_test_seed{42,123,2026}_cap{N}.json` and `I_tfidf_svm_ax_to_grind_test_cap{N}.json` + predictions siblings; checkpoints branch as `seed-<n>-length-cap-<N>` in the existing XLM-R staging repo |
