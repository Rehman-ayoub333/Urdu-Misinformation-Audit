...
- **I — IMPLEMENTED 2026-08-21, NO RESULTS YET.** Length ablation (`DECISION_REGISTER.md` M5-5): XLM-R × 3 seeds + best classical (B/tfidf_svm), Ax-to-Grind in-domain, word-cap sweep `{25, 50, 100, 200}` with 50 the required replication point. Both halves verified end to end by a CPU dry run; never run for real, so **no `I_*.json` exists and none may be quoted**. Bundled into the same single GPU trip as Q and M5-2's C/D/F predictions backfill.
