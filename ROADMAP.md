...
- **I, N, O — not started.** O and the shared error-analysis sample need no GPU; I needs a fresh XLM-R retrain.
**Files:** `run_cross_dataset.py`, `run_shortcut_analysis.py` (length-ablation mode; punctuation-ablation mode dispatches to `research/src/experiments/punctuation_ablation.py`), `research/src/explainability/integrated_gradients.py`, `research/src/evaluation/error_analysis.py`.
