...
- **Q — IMPLEMENTED 2026-08-21, NO RESULTS YET.** Punctuation-surface-form ablation (`DECISION_REGISTER.md` M2-3): XLM-R, Ax-to-Grind → Notri-Fact only, 3 seeds. The pipeline is built and verified end to end by a CPU dry run through both entry points; it has never been run for real, so **no `Q_*.json` exists and none may be quoted**. It needs the same fresh XLM-R retrain as I, and is bundled into one GPU trip with I and M5-2's C/D/F predictions backfill rather than three.
- **I — CLASSICAL HALF COMPLETE 2026-08-21; XLM-R half pending GPU.** Length ablation (`DECISION_REGISTER.md` M5-5), Ax-to-Grind in-domain, word-cap sweep `{25, 50, 100, 200}` with 50 the required replication point. **B/tfidf_svm measured, all four caps** (deterministic, no GPU, run locally on the pinned 3.12.11): macro-F1 **0.8551 / 0.8675 / 0.8726 / 0.8770** at caps 25/50/100/200 against uncapped B's 0.8835 — deltas **−0.0284 / −0.0160 / −0.0109 / −0.0065**, monotonically approaching the baseline as the cap loosens, all far above the 0.3333 majority floor. The XLM-R half (4 caps × 3 seeds) is implemented and dry-run-verified but **not run**, so no `I_xlm-roberta-base_*.json` exists and none may be quoted; it is bundled into the single GPU trip with Q and M5-2's C/D/F predictions backfill.
- **N, O — not started.** Both need no GPU beyond the M5-2 backfill; O and the shared error-analysis sample are unblocked once that lands.
**Files:** `run_cross_dataset.py`, `run_shortcut_analysis.py` (dispatches the punctuation-ablation and length-ablation modes to `research/src/experiments/punctuation_ablation.py` and `length_ablation.py`), `research/src/explainability/integrated_gradients.py`, `research/src/evaluation/error_analysis.py`.
**Real runs of Q and I require `--confirm-real-run`** (`DECISION_REGISTER.md` M5-6) — the CLI counterpart of the notebook's `CONFIRM_EVAL` guard. `--dry-run` is not gated.
**The remaining GPU work is bundled into one session:** `research/notebooks/06_milestone5_combined.ipynb` + `research/scripts/MILESTONE_5_GPU_HANDOFF_2.md` cover (a) the C/D predictions backfill, (b) the F-transformer predictions backfill, (c) Q's real 3-seed run and (d) I's XLM-R half (4 caps × 3 seeds). Estimated ~1.5–2.5 h against Kaggle's 12 h session cap and ~30 GPU-h/week quota, so it fits one session; the binding risks are local disk (~16.5 GB of checkpoints against a ~19.5 GB working quota) and private-repo Hub storage, both documented in the handoff.
